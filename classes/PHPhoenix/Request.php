...
class Request
{
	 ...
	 * @param \PHPhoenix\Route  $route  Route for this request
	 ...
	 *
	 */
	public function __construct($phoenix, $route, $method = "GET", $post = array(), $get = array(), $param=array(), $server = array(), $cookie = array())
	{
	...
	protected function get_filtered_value($params, $key = null, $default = null, $filter_xss=true)
	{
		if ($key == null)
			return $params;
		$val = $this->phoenix->arr($params, $key, $default);
	...
	public function server($key = null, $default = null)
	{
		if ($key == null)
			return $this->_server;
		return $this->phoenix->arr($this->_server, $key, $default);
	}

	/**
	 * Initializes the routed Controller and executes specified action
	 *
	 * @return \PHPhoenix\Response A Response object with the body and headers set
	 */
	public function execute()
	{
		$this->phoenix->cookie->set_cookie_data($this->_cookie);
		$class = $this->param('namespace',$this->phoenix->app_namespace).'Controller\\'.ucfirst($this->param('controller'));
		$controller = $this->phoenix->controller($class);
