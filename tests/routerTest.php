...
class RoterTest extends PHPUnit_Framework_TestCase
{
	...
	{
		$this->object=new \PHPhoenix\Router(new \PHPhoenix\Phoenix);
	}
	...
	{
		$except = false;
		$this->object->add(new \PHPhoenix\Route('/', 'a', 'b', array()));
		try {
			$this->object->get('c');
		} catch (\Exception $e) {
			$except = true;
	...
	{
		$this->object->add(new \PHPhoenix\Route('/', 'func', function() {
	...
	{
		$this->object->add(new \PHPhoenix\Route('/', 'default', '(/<controller>(/<action>(/<id>)))', array(
			'controller' => 'home',
			'action' => 'index'
		)));
	...
	{
		$this->object->add(new \PHPhoenix\Route('/', 'default', array('/<alpha><num>', array(
				'alpha' => '[a-z]*',
				'num' => '[0-9]*'
			)), array(
			'controller' => 'home',
	...
	public function testMatchMethod() {
		$this->object->add(new \PHPhoenix\Route('/', 'get', '/url', array(
			'controller' => 'home',
			'action' => 'get'
		), 'GeT'));
		$this->object->add(new \PHPhoenix\Route('/', 'post', '/url', array(
			'controller' => 'home',
			'action' => 'get'
		), array('PosT')));
